...
(function(process) {
  !function(global, factory) {
    ...
    function visitValue(value, visitor, context) {
      return Array.isArray(value) ? visitor.visitArray(value, context) : isStrictStringMap(value) ? visitor.visitStringMap(value, context) : null == value || isPrimitive(value) ? visitor.visitPrimitive(value, context) : visitor.visitOther(value, context);
    }
    ...
    function identifierModuleUrl(compileIdentifier) {
      var ref = compileIdentifier.reference;
      return ref instanceof StaticSymbol ? ref.filePath : reflector.importUri(ref);
    }
    function tokenName(token) {
      return isPresent(token.value) ? _sanitizeIdentifier(token.value) : identifierName(token.identifier);
    }
    function tokenReference(token) {
      return isPresent(token.identifier) ? token.identifier.reference : token.value;
    }
    ...
    function decodeSurrogatePairs(str, index) {
      ...
      if (high >= 55296 && high <= 57343 && str.length > index + 1) {
        ...
        if (low >= 56320 && low <= 57343)
          return 1024 * (high - 55296) + low - 56320 + 65536;
      }
      return high;
    }
    function add32(a, b) {
    ...
    function assetUrl(pkg, path, type) {
      return void 0 === path && (path = null), void 0 === type && (type = "src"), null == path ? "@angular/" + pkg + "/index" : "@angular/" + pkg + "/" + type + "/" + path;
    }
    ...
    function _resolveProviders(providers, providerType, eager, sourceSpan, targetErrors, targetProvidersByToken) {
      providers.forEach(function(provider) {
        ...
        if (isPresent(resolvedProvider) && !!resolvedProvider.multiProvider != !!provider.multi && targetErrors.push(new ProviderError("Mixing multi and non multi provider is not possible for token " + tokenName(resolvedProvider.token), sourceSpan)), resolvedProvider)
          ...
          resolvedProvider = new ProviderAst(provider.token, provider.multi, eager || lifecycleHooks.length > 0, [provider], providerType, lifecycleHooks, sourceSpan), targetProvidersByToken.set(tokenReference(provider.token), resolvedProvider);
        }
      });
    }
    ...
    function _normalizeStyleMetadata(entry, stateStyles, schema, errors, permitStateReferences) {
      ...
      (offset > 1 || offset < 0) && errors.push(new AnimationParseError("Offset values for animations must be between 0 and 1"));
    ...
    function importType(id, typeParams, typeModifiers) {
      return void 0 === typeParams && (typeParams = null), void 0 === typeModifiers && (typeModifiers = null), isPresent(id) ? expressionType(importExpr(id), typeParams, typeModifiers) : null;
    }
    function expressionType(expr, typeParams, typeModifiers) {
      return void 0 === typeParams && (typeParams = null), void 0 === typeModifiers && (typeModifiers = null), isPresent(expr) ? new ExpressionType(expr, typeParams, typeModifiers) : null;
    }
    ...
    function createCheckBindingField(builder) {
      ...
      return builder.fields.push(new ClassField(fieldExpr.name, null, [StmtModifier.Private])), builder.ctorStmts.push(THIS_EXPR.prop(fieldExpr.name).set(importExpr(createIdentifier(Identifiers.UNINITIALIZED))).toStmt()), new CheckBindingField(fieldExpr, bindingId);
    }
    ...
    function createDiTokenExpression(token) {
      return isPresent(token.value) ? literal(token.value) : importExpr(token.identifier);
    }
    function createInlineArray(values) {
      ...
      var log2 = Math.log(values.length) / Math.log(2),
          ...
          identifier = createIdentifier(identifierSpec);
    ...
    function createSharedBindingVariablesIfNeeded(stmts) {
      ...
      return readVars.has(VAL_UNWRAPPER_VAR.name) && unwrapperStmts.push(VAL_UNWRAPPER_VAR.set(importExpr(createIdentifier(Identifiers.ValueUnwrapper)).instantiate([])).toDeclStmt(null, [StmtModifier.Final])), unwrapperStmts;
    }
    ...
    function writeToRenderer(view, boundProp, renderElement, renderValue, logBindingUpdate, securityContextExpression) {
      ...
      switch (renderValue = sanitizedValue(view, boundProp, renderValue, securityContextExpression), boundProp.type) {
        case PropertyBindingType.Property:
          logBindingUpdate && updateStmts.push(importExpr(createIdentifier(Identifiers.setBindingDebugInfo)).callFn([renderer, renderElement, literal(boundProp.name), renderValue]).toStmt()), updateStmts.push(renderer.callMethod("setElementProperty", [renderElement, literal(boundProp.name), renderValue]).toStmt());
          break;
    ...
    function triggerAnimation(view, componentView, boundProp, boundOutputs, eventListener, renderElement, renderValue, lastRenderValue) {
      var detachStmts = [],
          ...
          emptyStateValue = literal(EMPTY_STATE),
          unitializedValue = importExpr(createIdentifier(Identifiers.UNINITIALIZED)),
      ...
      var registerStmts = [],
          animationStartMethodExists = boundOutputs.find(function(event) {
            return event.isAnimation && event.name == animationName && "start" == event.phase;
          });
      animationStartMethodExists && registerStmts.push(animationTransitionVar.callMethod("onStart", [eventListener.callMethod(BuiltinMethod.Bind, [view, literal(BoundEventAst.calcFullName(animationName, null, "start"))])]).toStmt());
      var animationDoneMethodExists = boundOutputs.find(function(event) {
        return event.isAnimation && event.name == animationName && "done" == event.phase;
      });
      return animationDoneMethodExists && registerStmts.push(animationTransitionVar.callMethod("onDone", [eventListener.callMethod(BuiltinMethod.Bind, [view, literal(BoundEventAst.calcFullName(animationName, null, "done"))])]).toStmt()), updateStmts.push.apply(updateStmts, registerStmts), detachStmts.push.apply(detachStmts, registerStmts), {
    ...
    function addCheckInputMethod(input, builder) {
      ...
      builder.genChanges && onChangeStatements.push(THIS_EXPR.prop(CHANGES_FIELD_NAME).key(literal(input)).set(importExpr(createIdentifier(Identifiers.SimpleChange)).instantiate([field.expression, CURR_VALUE_VAR])).toStmt());
    ...
    function addCheckHostMethod(hostProps, hostEvents, builder) {
      var stmts = [],
          methodParams = [new FnParam(VIEW_VAR.name, importType(createIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(COMPONENT_VIEW_VAR.name, importType(createIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(RENDER_EL_VAR.name, DYNAMIC_TYPE), new FnParam(THROW_ON_CHANGE_VAR.name, BOOL_TYPE)];
      hostProps.forEach(function(hostProp, hostPropIdx) {
        ...
        if (evalResult) {
          ...
          hostProp.needsRuntimeSecurityContext && (securityContextExpr = variable("secCtx_" + methodParams.length), methodParams.push(new FnParam(securityContextExpr.name, importType(createIdentifier(Identifiers.SecurityContext)))));
    ...
    function parseHostBindings(dirMeta, exprParser, schemaRegistry) {
      var errors = [],
          ...
          moduleUrl = identifierModuleUrl(dirMeta.type),
          sourceFileName = moduleUrl ? "in Directive " + identifierName(dirMeta.type) + " in " + moduleUrl : "in Directive " + identifierName(dirMeta.type),
    ...
    function getPropertyInView(property, callingView, definedView) {
      ...
      for (var viewProp = THIS_EXPR,
          currView = callingView; currView !== definedView && currView.declarationElement.view; )
    ...
    function bindInjectableDestroyLifecycleCallbacks(provider, providerInstance, compileElement) {
      ...
      onDestroyMethod.resetDebugInfo(compileElement.nodeIndex, compileElement.sourceAst), provider.providerType !== ProviderAstType.Directive && provider.providerType !== ProviderAstType.Component && provider.lifecycleHooks.indexOf(LifecycleHooks.OnDestroy) !== -1 && onDestroyMethod.addStmt(providerInstance.callMethod("ngOnDestroy", []).toStmt());
    }
    ...
    function bindQueryValues(ce) {
      ...
      ce.getProviderTokens().forEach(function(token) {
        ...
        if (queryWithRead.read.identifier)
          ...
          value = token ? ce.instances.get(tokenReference(token)) : ce.elementRef;
        }
        value && queryWithRead.query.addValue(value, ce.view);
      });
    }
    ...
    function finishView(view, targetStatements) {
      view.nodes.forEach(function(node) {
        node instanceof CompileElement && (node.finish(), node.hasEmbeddedView && finishView(node.embeddedView, targetStatements));
      }), view.finish(), createViewTopLevelStmts(view, targetStatements);
    }
    ...
    function createStaticNodeDebugInfo(node) {
      var compileElement = node instanceof CompileElement ? node : null,
          ...
          varTokenEntries = [];
      return isPresent(compileElement) && (providerTokens = compileElement.getProviderTokens().map(function(token) {
        return createDiTokenExpression(token);
      }), isPresent(compileElement.component) && (componentToken = createDiTokenExpression(identifierToken(compileElement.component.type))), Object.keys(compileElement.referenceTokens).forEach(function(varName) {
    ...
    function createViewClass(view, renderCompTypeVar, nodeDebugInfosVar) {
      ...
      view.genConfig.genDebugInfo && superConstructorArgs.push(nodeDebugInfosVar), view.viewType === ViewType.EMBEDDED && (viewConstructorArgs.push(new FnParam("declaredViewContainer", importType(createIdentifier(Identifiers.ViewContainer)))), superConstructorArgs.push(variable("declaredViewContainer")));
    ...
    function generateCreateEmbeddedViewsMethod(view) {
      var nodeIndexVar = variable("nodeIndex"),
          stmts = [];
      return view.nodes.forEach(function(node) {
        node instanceof CompileElement && node.embeddedView && stmts.push(new IfStmt(nodeIndexVar.equals(literal(node.nodeIndex)), [new ReturnStatement(node.embeddedView.classExpr.instantiate([ViewProperties.viewUtils, THIS_EXPR, literal(node.nodeIndex), node.renderNode, node.viewContainer]))]));
      }), stmts.length > 0 && stmts.push(new ReturnStatement(NULL_EXPR)), new ClassMethod("createEmbeddedViewInternal", [new FnParam(nodeIndexVar.name, NUMBER_TYPE)], stmts, importType(createIdentifier(Identifiers.AppView), [DYNAMIC_TYPE]));
    }
      ...
      return types.forEach(function(typeSummary) {
        if (serializer.addOrMergeSummary({
          ...
          ngModuleSummary.exportedDirectives.concat(ngModuleSummary.exportedPipes).forEach(function(id) {
            ...
            host.isSourceFile(symbol.filePath) || serializer.addOrMergeSummary(summaryResolver.resolveSummary(symbol));
          });
        }
    ...
    function createAotCompiler(compilerHost, options) {
      ...
      var htmlParser = new I18NHtmlParser(new HtmlParser, translations, options.i18nFormat),
          ...
          console = new Console,
          tmplParser = new TemplateParser(expressionParser, elementSchemaRegistry, htmlParser, console, []),
          resolver = new CompileMetadataResolver(new NgModuleResolver(staticReflector), new DirectiveResolver(staticReflector), new PipeResolver(staticReflector), summaryResolver, elementSchemaRegistry, normalizer, staticReflector),
    ...
    var StaticSymbol = function() {
      ...
      return StaticSymbol;
        ...
        ListWrapper = function() {
          ...
          return ListWrapper.findLast = function(arr, condition) {
        ...
        EMPTY_STATE = _angular_core.__core_private__.EMPTY_STATE,
    ...
    var NAMED_ENTITIES = {
      ...
      zwnj: "‌"
        ...
        BaseError = function(_super) {
          ...
          return __extends$3(BaseError, _super), Object.defineProperty(BaseError.prototype, "message", {
            get: function() {
        ...
        __extends$2 = (function(_super) {
          function WrappedError(message, error) {
            _super.call(this, message + " caused by: " + (error instanceof Error ? error.message : error)), this.originalError = error;
          }
          return __extends$3(WrappedError, _super), Object.defineProperty(WrappedError.prototype, "stack", {
            get: function() {
        ...
        }(BaseError), this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        SyntaxError = function(_super) {
          ...
          return __extends$2(SyntaxError, _super), SyntaxError;
        }(BaseError),
      ...
      return CompileStylesheetMetadata;
        ...
        TransitiveCompileNgModuleMetadata = function() {
          ...
          return TransitiveCompileNgModuleMetadata.prototype.addProvider = function(provider, module) {
            ...
            this.entryComponentsSet.has(id.reference) || (this.entryComponentsSet.add(id.reference), this.entryComponents.push(id));
        ...
        Quote = function(_super) {
          ...
          return __extends$4(Quote, _super), Quote.prototype.visit = function(visitor, context) {
        ...
        EmptyExpr = function(_super) {
          ...
          return __extends$4(EmptyExpr, _super), EmptyExpr.prototype.visit = function(visitor, context) {
            void 0 === context && (context = null);
          }, EmptyExpr;
        }(AST),
        ImplicitReceiver = function(_super) {
          ...
          return __extends$4(ImplicitReceiver, _super), ImplicitReceiver.prototype.visit = function(visitor, context) {
        ...
        Chain = function(_super) {
          ...
          return __extends$4(Chain, _super), Chain.prototype.visit = function(visitor, context) {
        ...
        Conditional = function(_super) {
          ...
          return __extends$4(Conditional, _super), Conditional.prototype.visit = function(visitor, context) {
        ...
        PropertyRead = function(_super) {
          ...
          return __extends$4(PropertyRead, _super), PropertyRead.prototype.visit = function(visitor, context) {
        ...
        PropertyWrite = function(_super) {
          ...
          return __extends$4(PropertyWrite, _super), PropertyWrite.prototype.visit = function(visitor, context) {
        ...
        SafePropertyRead = function(_super) {
          ...
          return __extends$4(SafePropertyRead, _super), SafePropertyRead.prototype.visit = function(visitor, context) {
        ...
        KeyedRead = function(_super) {
          ...
          return __extends$4(KeyedRead, _super), KeyedRead.prototype.visit = function(visitor, context) {
        ...
        KeyedWrite = function(_super) {
          ...
          return __extends$4(KeyedWrite, _super), KeyedWrite.prototype.visit = function(visitor, context) {
        ...
        BindingPipe = function(_super) {
          ...
          return __extends$4(BindingPipe, _super), BindingPipe.prototype.visit = function(visitor, context) {
        ...
        LiteralPrimitive = function(_super) {
          ...
          return __extends$4(LiteralPrimitive, _super), LiteralPrimitive.prototype.visit = function(visitor, context) {
        ...
        LiteralArray = function(_super) {
          ...
          return __extends$4(LiteralArray, _super), LiteralArray.prototype.visit = function(visitor, context) {
        ...
        LiteralMap = function(_super) {
          ...
          return __extends$4(LiteralMap, _super), LiteralMap.prototype.visit = function(visitor, context) {
        ...
        Interpolation = function(_super) {
          ...
          return __extends$4(Interpolation, _super), Interpolation.prototype.visit = function(visitor, context) {
        ...
        Binary = function(_super) {
          ...
          return __extends$4(Binary, _super), Binary.prototype.visit = function(visitor, context) {
        ...
        PrefixNot = function(_super) {
          ...
          return __extends$4(PrefixNot, _super), PrefixNot.prototype.visit = function(visitor, context) {
        ...
        MethodCall = function(_super) {
          ...
          return __extends$4(MethodCall, _super), MethodCall.prototype.visit = function(visitor, context) {
        ...
        SafeMethodCall = function(_super) {
          ...
          return __extends$4(SafeMethodCall, _super), SafeMethodCall.prototype.visit = function(visitor, context) {
        ...
        FunctionCall = function(_super) {
          ...
          return __extends$4(FunctionCall, _super), FunctionCall.prototype.visit = function(visitor, context) {
        ...
        ASTWithSource = function(_super) {
          ...
          return __extends$4(ASTWithSource, _super), ASTWithSource.prototype.visit = function(visitor, context) {
        ...
        __decorate$2 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
        ...
        __metadata$2 = this && this.__metadata || function(k, v) {
    ...
    var KEYWORDS = ["var", "let", "null", "undefined", "true", "false", "if", "else", "this"],
        ...
        __decorate$1 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$1 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
 ...
 while (operatorFound);
        ...
        ParseErrorLevel = {};
    ParseErrorLevel.WARNING = 0, ParseErrorLevel.FATAL = 1, ParseErrorLevel[ParseErrorLevel.WARNING] = "WARNING", ParseErrorLevel[ParseErrorLevel.FATAL] = "FATAL";
    ...
    var Token$1 = function() {
      ...
      return Token;
        ...
        TokenError = function(_super) {
          ...
          return __extends$6(TokenError, _super), TokenError;
        }(ParseError),
 ...
 while (!this._isTextEnd());
        ...
        TreeError = function(_super) {
          ...
          return __extends$5(TreeError, _super), TreeError.create = function(elementName, span, msg) {
        ...
        Message = function() {
          function Message(nodes, placeholders, placeholderToMessage, meaning, description) {
            this.nodes = nodes, this.placeholders = placeholders, this.placeholderToMessage = placeholderToMessage, this.meaning = meaning, this.description = description;
          }
          return Message;
        }(),
        Text$1 = function() {
        ...
        PlaceholderRegistry = function() {
          ...
          return PlaceholderRegistry.prototype.getStartTagPlaceholderName = function(tag, attrs, isVoid) {
            ...
            return this._placeHolderNameCounts[base] = next ? next + 1 : 1, next ? base + "_" + next : base;
          }, PlaceholderRegistry;
        }(),
        ...
        I18nError = function(_super) {
          ...
          return __extends$7(I18nError, _super), I18nError;
        }(ParseError),
        _I18N_ATTR = "i18n",
    ...
    var _Visitor = function() {
      ...
      return _Visitor.prototype.extract = function(nodes, interpolationConfig) {
        ...
        XmlParser = function(_super) {
          ...
          return __extends$8(XmlParser, _super), XmlParser.prototype.parse = function(source, url, parseExpansionForms) {
        ...
        _SerializerIgnoreIcuExpVisitor = function(_super) {
          ...
          return __extends$9(_SerializerIgnoreIcuExpVisitor, _super), _SerializerIgnoreIcuExpVisitor.prototype.visitIcu = function(icu, context) {
            var _this = this,
                strCases = Object.keys(icu.cases).map(function(k) {
                  return k + " {" + icu.cases[k].visit(_this) + "}";
                });
    ...
    var __extends$10 = this && this.__extends || function(d, b) {
      ...
      d.prototype = null === b ? Object.create(b) : (__.prototype = b.prototype, new __);
        ...
        CR = function(_super) {
          ...
          return __extends$10(CR, _super), CR;
        }(Text$2),
        ...
        Xliff = function() {
          function Xliff() {}
          return Xliff.prototype.write = function(messages) {
            var _this = this,
                visitor = new _WriteVisitor,
                visited = {},
                transUnits = [];
        ...
        _WriteVisitor = function() {
          ...
          return _WriteVisitor.prototype.visitText = function(text, context) {
            ...
            var _this = this;
            return this._isInIcu = !1, (_a = []).concat.apply(_a, nodes.map(function(node) {
              return node.visit(_this);
            }));
            var _a;
          }, _WriteVisitor;
        ...
        Xmb = function() {
          function Xmb() {}
          return Xmb.prototype.write = function(messages) {
            var _this = this,
                exampleVisitor = new ExampleVisitor,
        ...
        Xtb = function() {
          function Xtb() {}
          return Xtb.prototype.write = function(messages) {
            throw new Error("Unsupported");
        ...
        XtbParser = function() {
          ...
          return XtbParser.prototype.parse = function(xtb, url) {
            this._bundleDepth = 0, this._mlNodesByMsgId = {};
            var xml = (new XmlParser).parse(xtb, url, !0);
            ...
            switch (element.name) {
              ...
              case _TRANSLATION_TAG:
                ...
                if (idAttr) {
                  var id = idAttr.value;
                  this._mlNodesByMsgId.hasOwnProperty(id) ? this._addError(element, "Duplicated translations for msg " + id) : this._mlNodesByMsgId[id] = element.children;
                } else
        ...
        __decorate$3 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$3 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        HtmlParser = function(_super) {
          ...
          return __extends$11(HtmlParser, _super), HtmlParser.prototype.parse = function(source, url, parseExpansionForms, interpolationConfig) {
        ...
        TranslationBundle = function() {
          function TranslationBundle(_i18nNodesByMsgId, digest) {
            void 0 === _i18nNodesByMsgId && (_i18nNodesByMsgId = {}), this._i18nNodesByMsgId = _i18nNodesByMsgId, this.digest = digest, this._i18nToHtml = new I18nToHtmlVisitor(_i18nNodesByMsgId, digest);
          }
        ...
        I18nToHtmlVisitor = function() {
          function I18nToHtmlVisitor(_i18nNodesByMsgId, _digest) {
            void 0 === _i18nNodesByMsgId && (_i18nNodesByMsgId = {}), this._i18nNodesByMsgId = _i18nNodesByMsgId, this._digest = _digest, this._srcMsgStack = [], this._errors = [];
          }
          return I18nToHtmlVisitor.prototype.convert = function(srcMsg) {
            this._srcMsgStack.length = 0, this._errors.length = 0;
            ...
            var phName = ph.name;
            return this._srcMsg.placeholders.hasOwnProperty(phName) ? this._srcMsg.placeholders[phName] : this._srcMsg.placeholderToMessage.hasOwnProperty(phName) ? this._convertToText(this._srcMsg.placeholderToMessage[phName]) : (this._addError(ph, "Unknown placeholder"), "");
            ...
            var _this = this,
                digest = this._digest(srcMsg);
            if (this._i18nNodesByMsgId.hasOwnProperty(digest)) {
              this._srcMsgStack.push(this._srcMsg), this._srcMsg = srcMsg;
              var nodes = this._i18nNodesByMsgId[digest],
                  text = nodes.map(function(node) {
                    return node.visit(_this);
                  }).join("");
              return this._srcMsg = this._srcMsgStack.pop(), text;
            }
            return this._addError(srcMsg.nodes[0], "Missing translation for message " + digest), "";
        ...
        I18NHtmlParser = function() {
          ...
          return I18NHtmlParser.prototype.parse = function(source, url, parseExpansionForms, interpolationConfig) {
            ...
            var serializer = this._createSerializer(),
                translationBundle = TranslationBundle.load(this._translations, url, serializer);
        ...
        ExpansionError = function(_super) {
          ...
          return __extends$12(ExpansionError, _super), ExpansionError;
        }(ParseError),
        _Expander = function() {
          function _Expander() {
        ...
        ProviderError = function(_super) {
          ...
          return __extends$13(ProviderError, _super), ProviderError;
        }(ParseError),
        ProviderViewContext = function() {
          function ProviderViewContext(component, sourceSpan) {
            var _this = this;
            this.component = component, this.sourceSpan = sourceSpan, this.errors = [], this.viewQueries = _getViewQueries(component), this.viewProviders = new Map, component.viewProviders.forEach(function(provider) {
              isBlank(_this.viewProviders.get(tokenReference(provider.token))) && _this.viewProviders.set(tokenReference(provider.token), !0);
            });
          }
        ...
        ProviderElementContext = function() {
          function ProviderElementContext(viewContext, _parent, _isViewRoot, _directiveAsts, attrs, refs, _sourceSpan) {
            ...
            }), isPresent(queriedTokens.get(resolveIdentifier(Identifiers.ViewContainerRef))) && (this._hasViewContainer = !0), Array.from(this._allProviders.values()).forEach(function(provider) {
          ...
          return ProviderElementContext.prototype.afterElement = function() {
            ...
            this._getQueriesFor(token).forEach(function(query) {
              ...
              isBlank(queryReadTokens.get(tokenReference(queryReadToken))) && queryReadTokens.set(tokenReference(queryReadToken), !0);
            });
            ...
            if (dep.isSkipSelf || (result = this._getLocalDependency(requestingProviderType, dep, eager)), dep.isSelf)
              ...
              result || (result = !dep.isHost || this.viewContext.component.isHost || this.viewContext.component.type.reference === tokenReference(dep.token) || isPresent(this.viewContext.viewProviders.get(tokenReference(dep.token))) ? dep : dep.isOptional ? result = {
                isValue: !0,
                value: null
              } : null);
            }
        ...
        NgModuleProviderAnalyzer = function() {
          ...
          return NgModuleProviderAnalyzer.prototype.parse = function() {
            ...
            void 0 === eager && (eager = null);
            var foundLocal = !1;
            !dep.isSkipSelf && isPresent(dep.token) && (tokenReference(dep.token) === resolveIdentifier(Identifiers.Injector) || tokenReference(dep.token) === resolveIdentifier(Identifiers.ComponentFactoryResolver) ? foundLocal = !0 : isPresent(this._getOrCreateLocalProvider(dep.token, eager)) && (foundLocal = !0));
            var result = dep;
        ...
        BindingParser = function() {
          ...
          return BindingParser.prototype.createDirectiveHostPropertyAsts = function(dirMeta, sourceSpan) {
            ...
            if (boundProp.isAnimation)
              return new BoundElementPropertyAst(boundProp.name, PropertyBindingType.Animation, _angular_core.SecurityContext.NONE, (!1), boundProp.expression, null, boundProp.sourceSpan);
            var bindingType,
                securityContexts,
                unit = null,
            ...
            void 0 === level && (level = ParseErrorLevel.FATAL), this._targetErrors.push(new ParseError(sourceSpan, message, level));
            ...
            if (ast) {
              ...
              ast.visit(collector), collector.pipes.forEach(function(ast, pipeName) {
                _this.pipesByName.has(pipeName) || _this._reportError("The pipe '" + pipeName + "' could not be found", new ParseSourceSpan(sourceSpan.start.moveBy(ast.span.start), sourceSpan.start.moveBy(ast.span.end)));
              });
            }
            ...
            report.error && this._reportError(report.msg, sourceSpan, ParseErrorLevel.FATAL);
          }, BindingParser;
        }(),
        PipeCollector = function(_super) {
          function PipeCollector() {
            _super.apply(this, arguments), this.pipes = new Map;
          }
          return __extends$14(PipeCollector, _super), PipeCollector.prototype.visitPipe = function(ast, context) {
    ...
    var PreparsedElement = function() {
      ...
      return PreparsedElement;
        ...
        TemplateParser = function() {
          ...
          return TemplateParser.prototype.parse = function(component, template, directives, pipes, schemas, templateUrl) {
            ...
            if (htmlAstWithErrors.rootNodes.length > 0) {
              ...
              result = [];
            return this._assertNoReferenceDuplicationOnTemplate(result, errors), errors.length > 0 ? new TemplateParseResult(result, errors) : (this.transforms && this.transforms.forEach(function(transform) {
        ...
        TemplateParseVisitor = function() {
          ...
          return TemplateParseVisitor.prototype.visitExpansion = function(expansion, context) {
            ...
            element.attrs.forEach(function(attr) {
              ...
              normalizedName == TEMPLATE_ATTR ? templateBindingsSource = attr.value : normalizedName.startsWith(TEMPLATE_ATTR_PREFIX) && (templateBindingsSource = attr.value, prefixToken = normalizedName.substring(TEMPLATE_ATTR_PREFIX.length) + ":");
            ...
            directives.forEach(function(directive) {
              ...
              !isPresent(event.target) && allDirectiveEvents.has(event.name) || _this._reportError("Event binding " + event.fullName + ' not emitted by any directive on an embedded template. Make sure that the event name is spelled correctly and all directives are listed in the "@NgModule.declarations".', event.sourceSpan);
            });
        ...
        AnimationAst = function() {
          ...
          return AnimationAst.prototype.visit = function(visitor, context) {}, AnimationAst;
        }(),
        AnimationStateAst = function(_super) {
          function AnimationStateAst() {
            _super.apply(this, arguments);
          }
          return __extends$16(AnimationStateAst, _super), AnimationStateAst.prototype.visit = function(visitor, context) {}, AnimationStateAst;
        }(AnimationAst),
        AnimationEntryAst = function(_super) {
          ...
          return __extends$16(AnimationEntryAst, _super), AnimationEntryAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateDeclarationAst = function(_super) {
          ...
          return __extends$16(AnimationStateDeclarationAst, _super), AnimationStateDeclarationAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateTransitionAst = function(_super) {
          ...
          return __extends$16(AnimationStateTransitionAst, _super), AnimationStateTransitionAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStepAst = function(_super) {
          ...
          return __extends$16(AnimationStepAst, _super), AnimationStepAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStylesAst = function(_super) {
          ...
          return __extends$16(AnimationStylesAst, _super), AnimationStylesAst.prototype.visit = function(visitor, context) {
        ...
        AnimationKeyframeAst = function(_super) {
          ...
          return __extends$16(AnimationKeyframeAst, _super), AnimationKeyframeAst.prototype.visit = function(visitor, context) {
        ...
        AnimationWithStepsAst = function(_super) {
          ...
          return __extends$16(AnimationWithStepsAst, _super), AnimationWithStepsAst;
        }(AnimationAst),
        AnimationGroupAst = function(_super) {
          function AnimationGroupAst(steps) {
            _super.call(this, steps);
          }
          return __extends$16(AnimationGroupAst, _super), AnimationGroupAst.prototype.visit = function(visitor, context) {
        ...
        AnimationSequenceAst = function(_super) {
          ...
          return __extends$16(AnimationSequenceAst, _super), AnimationSequenceAst.prototype.visit = function(visitor, context) {
        ...
        __decorate$4 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$4 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        _INITIAL_KEYFRAME = 0,
        ...
        AnimationParseError = function(_super) {
          ...
          return __extends$15(AnimationParseError, _super), AnimationParseError.prototype.toString = function() {
            return "" + this.msg;
          }, AnimationParseError;
        }(ParseError),
        ...
        AnimationParser = function() {
          ...
          return AnimationParser.prototype.parseComponent = function(component) {
            var _this = this,
                errors = [],
                componentName = identifierName(component.type),
        ...
        __decorate$6 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$6 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
    ...
    var __decorate$5 = this && this.__decorate || function(decorators, target, key, desc) {
      ...
      return c > 3 && r && Object.defineProperty(target, key, r), r;
        ...
        __metadata$5 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ...
        __decorate$7 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$7 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        DirectiveResolver = function() {
          ...
          return DirectiveResolver.prototype.isDirective = function(type) {
            ...
            for (var names = new Set,
              ...
              names.has(name_1) || (names.add(name_1), reversedResult.push(binding));
            }
            ...
            var mergedInputs = this._dedupeBindings(directive.inputs ? directive.inputs.concat(inputs) : inputs),
                ...
                mergedHost = directive.host ? StringMapWrapper.merge(directive.host, host) : host,
    ...
    var Type$1 = function() {
      ...
      return Type.prototype.visitType = function(visitor, context) {}, Type.prototype.hasModifier = function(modifier) {
        return this.modifiers.indexOf(modifier) !== -1;
      }, Type;
    }(),
        BuiltinTypeName = {};
    BuiltinTypeName.Dynamic = 0, BuiltinTypeName.Bool = 1, BuiltinTypeName.String = 2, BuiltinTypeName.Int = 3, BuiltinTypeName.Number = 4, BuiltinTypeName.Function = 5, BuiltinTypeName.Null = 6, BuiltinTypeName[BuiltinTypeName.Dynamic] = "Dynamic", BuiltinTypeName[BuiltinTypeName.Bool] = "Bool", BuiltinTypeName[BuiltinTypeName.String] = "String", BuiltinTypeName[BuiltinTypeName.Int] = "Int", BuiltinTypeName[BuiltinTypeName.Number] = "Number", BuiltinTypeName[BuiltinTypeName.Function] = "Function", BuiltinTypeName[BuiltinTypeName.Null] = "Null";
    var BuiltinType = function(_super) {
      function BuiltinType(name, modifiers) {
        void 0 === modifiers && (modifiers = null), _super.call(this, modifiers), this.name = name;
      }
      return __extends$17(BuiltinType, _super), BuiltinType.prototype.visitType = function(visitor, context) {
        ...
        ExpressionType = function(_super) {
          ...
          return __extends$17(ExpressionType, _super), ExpressionType.prototype.visitType = function(visitor, context) {
        ...
        ArrayType = function(_super) {
          ...
          return __extends$17(ArrayType, _super), ArrayType.prototype.visitType = function(visitor, context) {
        ...
        MapType = function(_super) {
          ...
          return __extends$17(MapType, _super), MapType.prototype.visitType = function(visitor, context) {
    ...
    var Expression = function() {
      ...
      return Expression.prototype.visitExpression = function(visitor, context) {}, Expression.prototype.prop = function(name) {
    ...
    var ReadVarExpr = function(_super) {
      ...
      return __extends$17(ReadVarExpr, _super), ReadVarExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WriteVarExpr = function(_super) {
          ...
          return __extends$17(WriteVarExpr, _super), WriteVarExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WriteKeyExpr = function(_super) {
          ...
          return __extends$17(WriteKeyExpr, _super), WriteKeyExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WritePropExpr = function(_super) {
          ...
          return __extends$17(WritePropExpr, _super), WritePropExpr.prototype.visitExpression = function(visitor, context) {
    ...
    var InvokeMethodExpr = function(_super) {
      ...
      return __extends$17(InvokeMethodExpr, _super), InvokeMethodExpr.prototype.visitExpression = function(visitor, context) {
        ...
        InvokeFunctionExpr = function(_super) {
          ...
          return __extends$17(InvokeFunctionExpr, _super), InvokeFunctionExpr.prototype.visitExpression = function(visitor, context) {
        ...
        InstantiateExpr = function(_super) {
          ...
          return __extends$17(InstantiateExpr, _super), InstantiateExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralExpr = function(_super) {
          ...
          return __extends$17(LiteralExpr, _super), LiteralExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ExternalExpr = function(_super) {
          ...
          return __extends$17(ExternalExpr, _super), ExternalExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ConditionalExpr = function(_super) {
          ...
          return __extends$17(ConditionalExpr, _super), ConditionalExpr.prototype.visitExpression = function(visitor, context) {
        ...
        NotExpr = function(_super) {
          ...
          return __extends$17(NotExpr, _super), NotExpr.prototype.visitExpression = function(visitor, context) {
        ...
        CastExpr = function(_super) {
          ...
          return __extends$17(CastExpr, _super), CastExpr.prototype.visitExpression = function(visitor, context) {
        ...
        FunctionExpr = function(_super) {
          ...
          return __extends$17(FunctionExpr, _super), FunctionExpr.prototype.visitExpression = function(visitor, context) {
        ...
        BinaryOperatorExpr = function(_super) {
          ...
          return __extends$17(BinaryOperatorExpr, _super), BinaryOperatorExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ReadPropExpr = function(_super) {
          ...
          return __extends$17(ReadPropExpr, _super), ReadPropExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ReadKeyExpr = function(_super) {
          ...
          return __extends$17(ReadKeyExpr, _super), ReadKeyExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralArrayExpr = function(_super) {
          ...
          return __extends$17(LiteralArrayExpr, _super), LiteralArrayExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralMapExpr = function(_super) {
          ...
          return __extends$17(LiteralMapExpr, _super), LiteralMapExpr.prototype.visitExpression = function(visitor, context) {
    ...
    var Statement = function() {
      ...
      return Statement.prototype.visitStatement = function(visitor, context) {}, Statement.prototype.hasModifier = function(modifier) {
        ...
        DeclareVarStmt = function(_super) {
          ...
          return __extends$17(DeclareVarStmt, _super), DeclareVarStmt.prototype.visitStatement = function(visitor, context) {
        ...
        DeclareFunctionStmt = function(_super) {
          ...
          return __extends$17(DeclareFunctionStmt, _super), DeclareFunctionStmt.prototype.visitStatement = function(visitor, context) {
        ...
        ExpressionStatement = function(_super) {
          ...
          return __extends$17(ExpressionStatement, _super), ExpressionStatement.prototype.visitStatement = function(visitor, context) {
        ...
        ReturnStatement = function(_super) {
          ...
          return __extends$17(ReturnStatement, _super), ReturnStatement.prototype.visitStatement = function(visitor, context) {
        ...
        ClassField = function(_super) {
          ...
          return __extends$17(ClassField, _super), ClassField;
        }(AbstractClassPart),
        ClassMethod = function(_super) {
          ...
          return __extends$17(ClassMethod, _super), ClassMethod;
        }(AbstractClassPart),
        ClassGetter = function(_super) {
          ...
          return __extends$17(ClassGetter, _super), ClassGetter;
        }(AbstractClassPart),
        ClassStmt = function(_super) {
          ...
          return __extends$17(ClassStmt, _super), ClassStmt.prototype.visitStatement = function(visitor, context) {
        ...
        IfStmt = function(_super) {
          ...
          return __extends$17(IfStmt, _super), IfStmt.prototype.visitStatement = function(visitor, context) {
        ...
        TryCatchStmt = (function(_super) {
          ...
          return __extends$17(CommentStmt, _super), CommentStmt.prototype.visitStatement = function(visitor, context) {
          ...
          return __extends$17(TryCatchStmt, _super), TryCatchStmt.prototype.visitStatement = function(visitor, context) {
        ...
        ThrowStmt = function(_super) {
          ...
          return __extends$17(ThrowStmt, _super), ThrowStmt.prototype.visitStatement = function(visitor, context) {
        ...
        _ReplaceVariableTransformer = function(_super) {
          ...
          return __extends$17(_ReplaceVariableTransformer, _super), _ReplaceVariableTransformer.prototype.visitReadVarExpr = function(ast, context) {
        ...
        _VariableFinder = function(_super) {
          ...
          return __extends$17(_VariableFinder, _super), _VariableFinder.prototype.visitReadVarExpr = function(ast, context) {
    ...
    var _AstToIrVisitor = function() {
      ...
      return _AstToIrVisitor.prototype.visitBinary = function(ast, mode) {
        ...
        __decorate$8 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$8 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ...
        DirectiveWrapperCompiler = function() {
          ...
          return DirectiveWrapperCompiler.dirWrapperClassName = function(id) {
            ...
            }), addNgDoCheckMethod(builder), addCheckHostMethod(hostParseResult.hostProps, hostParseResult.hostListeners, builder), addHandleEventMethod(hostParseResult.hostListeners, builder), addSubscribeMethod(dirMeta, builder);
        ...
        __decorate$10 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$10 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ...
        __decorate$11 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$11 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ...
        __decorate$12 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$12 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        SummaryResolver = function() {
          function SummaryResolver() {}
          return SummaryResolver.prototype.resolveSummary = function(reference) {
            return null;
        ...
        __decorate$9 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$9 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ERROR_COLLECTOR_TOKEN = new _angular_core.OpaqueToken("ErrorCollector"),
        CompileMetadataResolver = function() {
          function CompileMetadataResolver(_ngModuleResolver, _directiveResolver, _pipeResolver, _summaryResolver, _schemaRegistry, _directiveNormalizer, _reflector, _errorCollector) {
            void 0 === _reflector && (_reflector = reflector), this._ngModuleResolver = _ngModuleResolver, this._directiveResolver = _directiveResolver, this._pipeResolver = _pipeResolver, this._summaryResolver = _summaryResolver, this._schemaRegistry = _schemaRegistry, this._directiveNormalizer = _directiveNormalizer, this._reflector = _reflector, this._errorCollector = _errorCollector, this._directiveCache = new Map, this._summaryCache = new Map, this._pipeCache = new Map, this._ngModuleCache = new Map, this._ngModuleOfTypes = new Map;
          }
          return CompileMetadataResolver.prototype.clearCacheFor = function(type) {
            var dirMeta = this._directiveCache.get(type);
            this._directiveCache.delete(type), this._summaryCache.delete(type), this._pipeCache.delete(type), this._ngModuleOfTypes.delete(type), this._ngModuleCache.clear(), dirMeta && this._directiveNormalizer.clearCacheFor(dirMeta);
          }, CompileMetadataResolver.prototype.clearCache = function() {
            this._directiveCache.clear(), this._summaryCache.clear(), this._pipeCache.clear(), this._ngModuleCache.clear(), this._ngModuleOfTypes.clear(), this._directiveNormalizer.clearCache();
            ...
            if (!this._directiveCache.has(directiveType)) {
              ...
              var _a = this.getNonNormalizedDirectiveMetadata(directiveType),
                  ...
                  createDirectiveMetadata = function(templateMetadata) {
                    ...
                    return _this._directiveCache.set(directiveType, normalizedDirMeta), _this._summaryCache.set(directiveType, normalizedDirMeta.toSummary()), normalizedDirMeta;
                  };
              if (metadata.isComponent) {
                ...
                return templateMeta.syncResult ? (createDirectiveMetadata(templateMeta.syncResult), null) : isSync ? (this._reportError(new ComponentStillLoadingError(directiveType), directiveType), null) : templateMeta.asyncResult.then(createDirectiveMetadata);
              }
            ...
            }).concat(entryComponentMetadata)), selector || (selector = this._schemaRegistry.getDefaultComponentElementName())) : selector || (this._reportError(new SyntaxError("Directive " + stringifyType(directiveType) + " has no selector, please add it!"), directiveType), selector = "error");
            var providers = [];
            isPresent(dirMeta.providers) && (providers = this._getProvidersMetadata(dirMeta.providers, entryComponentMetadata, 'providers for "' + stringifyType(directiveType) + '"', [], directiveType));
            var queries = [],
                viewQueries = [];
            isPresent(dirMeta.queries) && (queries = this._getQueriesMetadata(dirMeta.queries, !1, directiveType), viewQueries = this._getQueriesMetadata(dirMeta.queries, !0, directiveType));
            ...
            return dirMeta || this._reportError(new SyntaxError("Illegal state: getDirectiveMetadata can only be called after loadNgModuleMetadata for a module that declares it. Directive " + stringifyType(directiveType) + "."), directiveType), dirMeta;
          }, CompileMetadataResolver.prototype.getDirectiveSummary = function(dirType) {
            var dirSummary = this._loadSummary(dirType, CompileSummaryKind.Directive);
            return dirSummary || this._reportError(new SyntaxError("Illegal state: Could not load the summary for directive " + stringifyType(dirType) + "."), dirType), dirSummary;
            ...
            meta.imports && flattenAndDedupeArray(meta.imports).forEach(function(importedType) {
              ...
              if (!importedModuleType)
                return void _this._reportError(new SyntaxError("Unexpected value '" + stringifyType(importedType) + "' imported by the module '" + stringifyType(moduleType) + "'"), moduleType);
              var importedModuleSummary = _this.getNgModuleSummary(importedModuleType);
              return importedModuleSummary ? void importedModules.push(importedModuleSummary) : void _this._reportError(new SyntaxError("Unexpected " + _this._getTypeDescriptor(importedType) + " '" + stringifyType(importedType) + "' imported by the module '" + stringifyType(moduleType) + "'"), moduleType);
            }), meta.exports && flattenAndDedupeArray(meta.exports).forEach(function(exportedType) {
              if (!isValidType(exportedType))
                return void _this._reportError(new SyntaxError("Unexpected value '" + stringifyType(exportedType) + "' exported by the module '" + stringifyType(moduleType) + "'"), moduleType);
            ...
            meta.declarations && flattenAndDedupeArray(meta.declarations).forEach(function(declaredType) {
              if (!isValidType(declaredType))
                return void _this._reportError(new SyntaxError("Unexpected value '" + stringifyType(declaredType) + "' declared by the module '" + stringifyType(moduleType) + "'"), moduleType);
              ...
              if (_this._directiveResolver.isDirective(declaredType))
                ...
                if (!_this._pipeResolver.isPipe(declaredType))
                  return void _this._reportError(new SyntaxError("Unexpected " + _this._getTypeDescriptor(declaredType) + " '" + stringifyType(declaredType) + "' declared by the module '" + stringifyType(moduleType) + "'"), moduleType);
            ...
            return exportedNonModuleIdentifiers.forEach(function(exportedId) {
              ...
              return isValidType(type) ? void bootstrapComponents.push(_this._getIdentifierMetadata(type)) : void _this._reportError(new SyntaxError("Unexpected value '" + stringifyType(type) + "' used in the bootstrap property of module '" + stringifyType(moduleType) + "'"), moduleType);
            }), entryComponents.push.apply(entryComponents, bootstrapComponents), meta.schemas && schemas.push.apply(schemas, flattenAndDedupeArray(meta.schemas)), compileMeta = new CompileNgModuleMetadata({
            ...
            return pipeMeta || this._reportError(new SyntaxError("Illegal state: getPipeMetadata can only be called after loadNgModuleMetadata for a module that declares it. Pipe " + stringifyType(pipeType) + "."), pipeType), pipeMeta;
          }, CompileMetadataResolver.prototype.getPipeSummary = function(pipeType) {
            var pipeSummary = this._loadSummary(pipeType, CompileSummaryKind.Pipe);
            return pipeSummary || this._reportError(new SyntaxError("Illegal state: Could not load the summary for pipe " + stringifyType(pipeType) + "."), pipeType), pipeSummary;
            ...
            var _this = this,
                ...
                dependenciesMetadata = params.map(function(param) {
                  ...
                  }) : token = param, isBlank(token) ? (hasUnknownDeps = !0, null) : {
            ...
            return provider.useFactory || provider.useExisting || provider.useClass ? (this._reportError(new SyntaxError("The ANALYZE_FOR_ENTRY_COMPONENTS token only supports useValue!"), type), []) : provider.multi ? (extractIdentifiers(provider.useValue, collectedIdentifiers), collectedIdentifiers.forEach(function(identifier) {
              (_this._directiveResolver.isDirective(identifier.reference) || _this._loadSummary(identifier.reference, CompileSummaryKind.Directive)) && components.push(identifier);
            }), components) : (this._reportError(new SyntaxError("The ANALYZE_FOR_ENTRY_COMPONENTS token only supports 'multi = true'!"), type), []);
            ...
            }) : (q.selector || this._reportError(new SyntaxError("Can't construct a query for the property \"" + propertyName + '" of "' + stringifyType(typeOrFunc) + "\" since the query selector wasn't defined."), typeOrFunc), selectors = [this._getTokenMetadata(q.selector)]), {
              selectors: selectors,
              first: q.first,
        ...
        _CompileValueConverter = function(_super) {
          ...
          return __extends$18(_CompileValueConverter, _super), _CompileValueConverter.prototype.visitOther = function(value, targetIdentifiers) {
        ...
        __decorate$13 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$13 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ...
        NgModuleCompiler = function() {
          ...
          return NgModuleCompiler.prototype.compile = function(ngModuleMeta, extraProviders) {
            var moduleUrl = identifierModuleUrl(ngModuleMeta.type),
                sourceFileName = isPresent(moduleUrl) ? "in NgModule " + identifierName(ngModuleMeta.type) + " in " + moduleUrl : "in NgModule " + identifierName(ngModuleMeta.type),
                sourceFile = new ParseSourceFile("", sourceFileName),
                sourceSpan = new ParseSourceSpan(new ParseLocation(sourceFile, null, null, null), new ParseLocation(sourceFile, null, null, null)),
                deps = [],
                ...
                entryComponentFactories = ngModuleMeta.transitiveModule.entryComponents.map(function(entryComponent) {
                  ...
                  }) && bootstrapComponentFactories.push(id), deps.push(new ComponentFactoryDependency(entryComponent, id)), id;
                }),
        ...
        _InjectorBuilder = function() {
          ...
          return _InjectorBuilder.prototype.addProvider = function(resolvedProvider) {
            ...
            resolvedProvider.lifecycleHooks.indexOf(LifecycleHooks.OnDestroy) !== -1 && this._destroyStmts.push(instance.callMethod("ngOnDestroy", []).toStmt()), this._tokens.push(resolvedProvider.token), this._instances.set(tokenReference(resolvedProvider.token), instance);
            ...
            var result = null;
            if (dep.isValue && (result = literal(dep.value)), dep.isSkipSelf || (!dep.token || tokenReference(dep.token) !== resolveIdentifier(Identifiers.Injector) && tokenReference(dep.token) !== resolveIdentifier(Identifiers.ComponentFactoryResolver) || (result = THIS_EXPR), result || (result = this._instances.get(tokenReference(dep.token)))), !result) {
        ...
        AbstractEmitterVisitor = function() {
          ...
          return AbstractEmitterVisitor.prototype.visitExpressionStmt = function(stmt, ctx) {
            ...
            return ctx.print("{", useNewLine), ctx.incIndent(), this.visitAllObjects(function(entry) {
              ctx.print(escapeIdentifier(entry.key, _this._escapeDollarInStrings, entry.quoted) + ": "), entry.value.visitExpression(_this, ctx);
        ...
        _TsEmitterVisitor = function(_super) {
          ...
          return __extends$19(_TsEmitterVisitor, _super), _TsEmitterVisitor.prototype.visitType = function(t, ctx, defaultType) {
            ...
            var _this = this;
            return ast.value.visitExpression(this, ctx), isPresent(ast.typeParams) && ast.typeParams.length > 0 && (ctx.print("<"), this.visitAllObjects(function(type) {
              return type.visitType(_this, ctx);
            }, ast.typeParams, ctx, ","), ctx.print(">")), null;
            ...
            var _this = this,
                ...
                moduleUrl = identifierModuleUrl(value);
            if (isBlank(name))
              throw new Error("Internal error: unknown identifier " + value);
            if (isPresent(moduleUrl) && moduleUrl != this._moduleUrl) {
              var prefix = this.importsWithPrefixes.get(moduleUrl);
              isBlank(prefix) && (prefix = "import" + this.importsWithPrefixes.size, this.importsWithPrefixes.set(moduleUrl, prefix)), ctx.print(prefix + ".");
            }
            value.reference && value.reference.members && value.reference.members.length ? (ctx.print(value.reference.name), ctx.print("."), ctx.print(value.reference.members.join("."))) : ctx.print(name), isPresent(typeParams) && typeParams.length > 0 && (ctx.print("<"), this.visitAllObjects(function(type) {
              return type.visitType(_this, ctx);
    ...
    var __extends$20 = this && this.__extends || function(d, b) {
      ...
      d.prototype = null === b ? Object.create(b) : (__.prototype = b.prototype, new __);
        ...
        __decorate$14 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$14 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        BOOLEAN = "boolean",
        ...
        __decorate$15 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$15 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ...
        CompileMethod = function() {
          ...
          return CompileMethod.prototype._updateDebugContextIfNeeded = function() {
            if (this._newState.nodeIndex !== this._currState.nodeIndex || this._newState.sourceAst !== this._currState.sourceAst) {
              ...
              expr && this._bodyStatements.push(expr.toStmt());
            }
        ...
        __extends$22 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        _ReplaceViewTransformer = function(_super) {
          ...
          return __extends$22(_ReplaceViewTransformer, _super), _ReplaceViewTransformer.prototype._isThis = function(expr) {
        ...
        CompileQuery = function() {
          ...
          return CompileQuery.prototype.addValue = function(value, view) {
            for (var currentView = view,
                elPath = []; currentView && currentView !== this.view; ) {
        ...
        ViewClassDependency = function() {
          function ViewClassDependency(comp, name, placeholder) {
            this.comp = comp, this.name = name, this.placeholder = placeholder;
          }
        ...
        DirectiveWrapperDependency = function() {
          function DirectiveWrapperDependency(dir, name, placeholder) {
            this.dir = dir, this.name = name, this.placeholder = placeholder;
          }
          return DirectiveWrapperDependency;
        }(),
        __extends$21 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        CompileElement = function(_super) {
          function CompileElement(parent, view, nodeIndex, renderNode, sourceAst, component, _directives, _resolvedProvidersArray, hasViewContainer, hasEmbeddedView, references) {
            ...
            }), this.elementRef = importExpr(createIdentifier(Identifiers.ElementRef)).instantiate([this.renderNode]), this.instances.set(resolveIdentifier(Identifiers.ElementRef), this.elementRef), this.instances.set(resolveIdentifier(Identifiers.Injector), THIS_EXPR.callMethod("injector", [literal(this.nodeIndex)])), this.instances.set(resolveIdentifier(Identifiers.Renderer), THIS_EXPR.prop("renderer")), (this.hasViewContainer || this.hasEmbeddedView) && this._createViewContainer(), this.component && this._createComponentFactoryResolver();
          }
          return __extends$21(CompileElement, _super), CompileElement.createNull = function() {
            ...
            this.view.createMethod.addStmt(statement), this.viewContainer = THIS_EXPR.prop(fieldName), this.instances.set(resolveIdentifier(Identifiers.ViewContainer), this.viewContainer), this.view.viewContainers.push(this.viewContainer);
            ...
            var _this = this,
                entryComponents = this.component.entryComponents.map(function(entryComponent) {
                  var id = {reference: null};
                  return _this.view.targetDependencies.push(new ComponentFactoryDependency$1(entryComponent, id)), id;
                });
            if (entryComponents && 0 !== entryComponents.length) {
              ...
              })), injectFromViewParentInjector(this.view, createIdentifierToken(Identifiers.ComponentFactoryResolver), !1)]),
                  provider = {
                    token: createIdentifierToken(Identifiers.ComponentFactoryResolver),
                    useValue: createComponentFactoryResolverExpr
                  };
              this._resolvedProvidersArray.unshift(new ProviderAst(provider.token, (!1), (!0), [provider], ProviderAstType.PrivateService, [], this.sourceAst.sourceSpan));
            }
            ...
            if (this.embeddedView = embeddedView, isPresent(embeddedView)) {
              ...
              this._resolvedProvidersArray.unshift(new ProviderAst(provider.token, (!1), (!0), [provider], ProviderAstType.Builtin, [], this.sourceAst.sourceSpan));
            }
          }, CompileElement.prototype.beforeChildren = function() {
            var _this = this;
            this.hasViewContainer && this.instances.set(resolveIdentifier(Identifiers.ViewContainerRef), this.viewContainer.prop("vcRef")), this._resolvedProviders = new Map, this._resolvedProvidersArray.forEach(function(provider) {
                  ...
                  providerValueExpressions = resolvedProvider.providers.map(function(provider) {
                    ...
                    if (provider.useClass) {
                      ...
                      if (isDirectiveWrapper) {
                        ...
                        return _this.view.targetDependencies.push(new DirectiveWrapperDependency(provider.useClass, DirectiveWrapperCompiler.dirWrapperClassName(provider.useClass), directiveWrapperIdentifier)), DirectiveWrapperExpressions.create(directiveWrapperIdentifier, depsExpr);
                      }
                      return importExpr(provider.useClass).instantiate(depsExpr, importType(provider.useClass));
                    }
                    return convertValueToOutputAst(provider.useValue);
                  }),
                  propName = "_" + tokenName(resolvedProvider.token) + "_" + _this.nodeIndex + "_" + _this.instances.size,
                  instance = createProviderProperty(propName, providerValueExpressions, resolvedProvider.multiProvider, resolvedProvider.eager, _this);
              isDirectiveWrapper ? (_this.directiveWrapperInstance.set(tokenReference(resolvedProvider.token), instance), _this.instances.set(tokenReference(resolvedProvider.token), DirectiveWrapperExpressions.context(instance))) : _this.instances.set(tokenReference(resolvedProvider.token), instance);
            });
            for (var _loop_1 = function(i) {
              var directive = this_1._directives[i],
                  directiveInstance = this_1.instances.get(tokenReference(identifierToken(directive.type)));
            ...
            Object.keys(this.referenceTokens).forEach(function(varName) {
              ...
              varValue = token ? _this.instances.get(tokenReference(token)) : _this.renderNode, _this.view.locals.set(varName, varValue);
            });
          }, CompileElement.prototype.afterChildren = function(childNodeCount) {
            var _this = this;
            Array.from(this._resolvedProviders.values()).forEach(function(resolvedProvider) {
              var providerExpr = _this.instances.get(tokenReference(resolvedProvider.token)),
                  providerChildNodeCount = resolvedProvider.providerType === ProviderAstType.PrivateService ? 0 : childNodeCount;
            ...
            for (var queries,
              ...
              queries = currentEl._queries.get(tokenReference(token)), isPresent(queries) && result.push.apply(result, queries.filter(function(query) {
                return query.meta.descendants || distance <= 1;
              })), currentEl._directives.length > 0 && distance++, currentEl = currentEl.parent;
            return queries = this.view.componentView.viewQueries.get(tokenReference(token)), isPresent(queries) && result.push.apply(result, queries), result;
          }, CompileElement.prototype._addQuery = function(queryMeta, directiveInstance) {
            var propName = "_query_" + tokenName(queryMeta.selectors[0]) + "_" + this.nodeIndex + "_" + this._queryCount++,
                queryList = createQueryList(propName, this.view),
            ...
            var result = null;
            if (isPresent(dep.token)) {
              if (!result && tokenReference(dep.token) === resolveIdentifier(Identifiers.ChangeDetectorRef))
                return requestingProviderType === ProviderAstType.Component ? this.compViewExpr.prop("ref") : getPropertyInView(THIS_EXPR.prop("ref"), this.view, this.view.componentView);
              if (!result) {
            ...
            for (dep.isValue && (result = literal(dep.value)), result || dep.isSkipSelf || (result = this._getLocalDependency(requestingProviderType, dep)); !result && !currElement.parent.isNull(); )
              currElement = currElement.parent, result = currElement._getLocalDependency(ProviderAstType.PublicService, {token: dep.token});
        ...
        CompilePipe = function() {
          function CompilePipe(view, meta) {
            ...
            var deps = this.meta.type.diDeps.map(function(diDep) {
              return tokenReference(diDep.token) === resolveIdentifier(Identifiers.ChangeDetectorRef) ? getPropertyInView(THIS_EXPR.prop("ref"), _this.view, _this.view.componentView) : injectFromViewParentInjector(view, diDep.token, !1);
            });
          ...
          return CompilePipe.call = function(view, name, args) {
            ...
            if (this.meta.pure) {
              ...
              }), importExpr(createIdentifier(Identifiers.castByValue)).callFn([purePipeProxyInstance, pipeInstanceSeenFromPureProxy.prop("transform")]).callFn(args);
            }
            return getPropertyInView(this.instance, callingView, this.view).callMethod("transform", args);
          }, CompilePipe;
        }(),
        CompileViewRootNodeType = {};
    CompileViewRootNodeType.Node = 0, CompileViewRootNodeType.ViewContainer = 1, CompileViewRootNodeType.NgContent = 2, CompileViewRootNodeType[CompileViewRootNodeType.Node] = "Node", CompileViewRootNodeType[CompileViewRootNodeType.ViewContainer] = "ViewContainer", CompileViewRootNodeType[CompileViewRootNodeType.NgContent] = "NgContent";
    var CompileViewRootNode = function() {
      ...
      return CompileViewRootNode;
            ...
            if (this.viewType === ViewType.COMPONENT) {
              ...
              this.component.viewQueries.forEach(function(queryMeta, queryIndex) {
                var propName = "_viewQuery_" + tokenName(queryMeta.selectors[0]) + "_" + queryIndex,
                    queryList = createQueryList(propName, _this),
        ...
        _QueryWithRead = (not(DetectChangesVars.throwOnChange), function() {
        ...
        ViewBinderVisitor = function() {
          ...
          return ViewBinderVisitor.prototype.visitBoundText = function(ast, parent) {
            ...
            var hasEvents = bindOutputs(ast.outputs, ast.directives, compileElement, !0);
            return bindRenderInputs(ast.inputs, ast.outputs, hasEvents, compileElement), ast.directives.forEach(function(directiveAst, dirIndex) {
            ...
            return bindQueryValues(compileElement), bindOutputs(ast.outputs, ast.directives, compileElement, !1), ast.directives.forEach(function(directiveAst, dirIndex) {
              var directiveInstance = compileElement.instances.get(directiveAst.directive.type.reference),
                  directiveWrapperInstance = compileElement.directiveWrapperInstance.get(directiveAst.directive.type.reference);
        ...
        __decorate$16 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$16 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
        ...
        ViewCompiler = function() {
          ...
          return ViewCompiler.prototype.compileComponent = function(component, template, styles, pipes, compiledAnimations) {
            var dependencies = [],
                view = new CompileView(component, this._genConfig, pipes, styles, compiledAnimations, 0, CompileElement.createNull(), [], dependencies),
                statements = [];
        ...
        _PREVIOUS_ANIMATION_PLAYERS = variable("previousPlayers"),
        _EMPTY_MAP = literalMap([]),
        _EMPTY_ARRAY = literalArr([]),
        _AnimationBuilder = function() {
          ...
          return _AnimationBuilder.prototype.visitAnimationStyles = function(ast, context) {
            ...
            return context.isExpectingFirstStyleStep && (stylesArr.push(_ANIMATION_START_STATE_STYLES_VAR), context.isExpectingFirstStyleStep = !1), ast.styles.forEach(function(entry) {
              ...
              stylesArr.push(literalMap(entries, null, !0));
            }), importExpr(createIdentifier(Identifiers.AnimationStyles)).instantiate([importExpr(createIdentifier(Identifiers.collectAndResolveStyles)).callFn([_ANIMATION_COLLECTED_STYLES, literalArr(stylesArr)])]);
            ...
            var _this = this,
                ...
                keyframesExpr = importExpr(createIdentifier(Identifiers.balanceAnimationKeyframes)).callFn([_ANIMATION_COLLECTED_STYLES, _ANIMATION_END_STATE_STYLES_VAR, literalArr(keyframeExpressions)]);
            ...
            return context.isExpectingFirstAnimateStep && (previousStylesValue = _PREVIOUS_ANIMATION_PLAYERS, context.isExpectingFirstAnimateStep = !1), context.totalTransitionTime += ast.duration + ast.delay, _ANIMATION_FACTORY_RENDERER_VAR.callMethod("animate", [_ANIMATION_FACTORY_ELEMENT_VAR, startingStylesExpr, keyframesExpr, literal(ast.duration), literal(ast.delay), literal(ast.easing), previousStylesValue]);
            ...
            _isEndStateAnimateStep(lastStep) && (context.endStateAnimateStep = lastStep), context.totalTransitionTime = 0, context.isExpectingFirstStyleStep = !0, context.isExpectingFirstAnimateStep = !0;
            ...
            return ast.stateTransitions.forEach(function(transAst) {
            ...
            Object.keys(context.stateMap.states).forEach(function(stateName) {
              var value = context.stateMap.states[stateName],
                  variableValue = _EMPTY_MAP;
              if (isPresent(value)) {
                var styleMap_1 = [];
                Object.keys(value).forEach(function(key) {
                  styleMap_1.push([key, literal(value[key])]);
                }), variableValue = literalMap(styleMap_1, null, !0);
              }
              lookupMap.push([stateName, variableValue]);
            });
            var compiledStatesMapStmt = this._statesMapVar.set(literalMap(lookupMap, null, !0)).toDeclStmt(),
        ...
        _AnimationBuilderContext = function() {
          function _AnimationBuilderContext() {
            this.stateMap = new _AnimationBuilderStateMap, this.endStateAnimateStep = null, this.isExpectingFirstStyleStep = !1, this.isExpectingFirstAnimateStep = !1, this.totalTransitionTime = 0;
          }
        ...
        __extends$23 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        Serializer = function(_super) {
          function Serializer(host) {
            _super.call(this), this.host = host, this.symbols = [], this.indexBySymbol = new Map, this.processedSummaryBySymbol = new Map, this.processedSummaries = [];
          }
          return __extends$23(Serializer, _super), Serializer.prototype.addOrMergeSummary = function(summary) {
            ...
            if (value instanceof StaticSymbol) {
              ...
              return null == index && (index = this.indexBySymbol.size, this.indexBySymbol.set(value, index), this.symbols.push(value)), {__symbol: index};
            }
          }, Serializer;
        }(ValueTransformer),
        Deserializer = function(_super) {
          function Deserializer(symbolCache) {
            _super.call(this), this.symbolCache = symbolCache;
          }
          return __extends$23(Deserializer, _super), Deserializer.prototype.deserialize = function(json) {
            var _this = this,
                data = JSON.parse(json);
            return this.symbols = data.symbols.map(function(serializedSymbol) {
              return _this.symbolCache.get(serializedSymbol.filePath, serializedSymbol.name);
            }), visitValue(data.summaries, this, null);
          }, Deserializer.prototype.visitStringMap = function(map, context) {
            return "__symbol" in map ? this.symbols[map.__symbol] : _super.prototype.visitStringMap.call(this, map, context);
          }, Deserializer;
        }(ValueTransformer),
        AotCompiler = function() {
          ...
          return AotCompiler.prototype.clearCache = function() {
            ...
            var _this = this,
                ...
                exportedVars = [],
                generatedFiles = [];
            if (generatedFiles.push(this._createSummary(srcFileUrl, directives, pipes, ngModules, injectables)), exportedVars.push.apply(exportedVars, ngModules.map(function(ngModuleType) {
            ...
            var _this = this,
                ...
                json = serializeSummaries(this._host, this._summaryResolver, this._symbolResolver, symbolSummaries, typeSummaries);
            return new GeneratedFile(srcFileUrl, summaryFileName(srcFileUrl), json);
          }, AotCompiler.prototype._compileModule = function(ngModuleType, targetStatements) {
            var _this = this,
                ngModule = this._metadataResolver.getNgModuleMetadata(ngModuleType),
                providers = [];
            ...
            var _this = this,
                ...
                parsedTemplate = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, ngModule.schemas, identifierName(compMeta.type)),
                stylesExpr = componentStyles ? variable(componentStyles.stylesVar) : literalArr([]),
                compiledAnimations = this._animationCompiler.compile(identifierName(compMeta.type), parsedAnimations),
                viewResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, stylesExpr, pipes, compiledAnimations);
            return componentStyles && targetStatements.push.apply(targetStatements, _resolveStyleStatements(this._symbolResolver, componentStyles, fileSuffix)), compiledAnimations.forEach(function(entry) {
              return targetStatements.push.apply(targetStatements, entry.statements);
            }), targetStatements.push.apply(targetStatements, _resolveViewStatements(this._symbolResolver, viewResult)), viewResult.viewClassVar;
        ...
        StaticAndDynamicReflectionCapabilities = function() {
          function StaticAndDynamicReflectionCapabilities(staticDelegate) {
            this.staticDelegate = staticDelegate, this.dynamicDelegate = new ReflectionCapabilities;
          }
        ...
        __extends$24 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
          ...
          return StaticReflector.prototype.importUri = function(typeOrFunc) {
            ...
            try {
              ...
              if (!parameters_1) {
                ...
                if (ctorData) {
                  ...
                  classMetadata.extends && (parameters_1 = this.parameters(this.simplify(type, classMetadata.extends)));
            ...
            function simplifyInContext(context, value, depth) {
              ...
              function simplifyCall(functionSymbol, targetFunction, args) {
                ...
                return 0 === depth ? {__symbolic: "ignore"} : simplify({
                  __symbolic: "error",
                  ...
                  if (expression.__symbolic) {
                    ...
                    switch (expression.__symbolic) {
                      ...
                      case "index":
                        ...
                        return indexTarget && isPrimitive$1(index) ? indexTarget[index] : null;
                      case "select":
                        ...
                        return selectTarget && isPrimitive$1(member) ? simplifyInContext(selectContext, selectTarget[member], depth + 1) : null;
                      case "reference":
                        var name_1 = expression.name,
                            localValue = scope.resolve(name_1);
                      ...
                      case "error":
                        ...
                        if (expression.line)
                          throw message = message + " (position " + (expression.line + 1) + ":" + (expression.character + 1) + " in the original .ts file)", positionalError(message, context.filePath, expression.line, expression.character);
        ...
        PopulatedScope = function(_super) {
          function PopulatedScope(bindings) {
            _super.call(this), this.bindings = bindings;
          }
          return __extends$24(PopulatedScope, _super), PopulatedScope.prototype.resolve = function(name) {
            return this.bindings.has(name) ? this.bindings.get(name) : BindingScope.missing;
          }, PopulatedScope;
        }(BindingScope),
        __extends$25 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        StaticSymbolResolver = function() {
          function StaticSymbolResolver(host, staticSymbolCache, summaryResolver, errorRecorder) {
            this.host = host, this.staticSymbolCache = staticSymbolCache, this.summaryResolver = summaryResolver, this.errorRecorder = errorRecorder, this.metadataCache = new Map, this.resolvedSymbols = new Map, this.resolvedFilePaths = new Set;
          }
          return StaticSymbolResolver.prototype.resolveSymbol = function(staticSymbol) {
            ...
            return result || (this._createSymbolsOf(staticSymbol.filePath), result = this.resolvedSymbols.get(staticSymbol)), result;
            ...
            if (!this.resolvedFilePaths.has(filePath)) {
              ...
              if (metadata.metadata && Object.keys(metadata.metadata).forEach(function(symbolName) {
                ...
                for (var _loop_1 = function(moduleExport) {
                  if (moduleExport.export)
                    moduleExport.export.forEach(function(exportSymbol) {
                      var symbolName;
                      symbolName = "string" == typeof exportSymbol ? exportSymbol : exportSymbol.as;
                      var symName = symbolName;
                      "string" != typeof exportSymbol && (symName = exportSymbol.name);
            ...
            var self = this,
                ReferenceTransformer = function(_super) {
                  ...
                  return __extends$25(ReferenceTransformer, _super), ReferenceTransformer.prototype.visitStringMap = function(map, functionParams) {
                    ...
                    if ("reference" === symbolic) {
                      ...
                      var filePath = void 0;
                      if (module_1) {
                        if (filePath = self.resolveModule(module_1, sourceSymbol.filePath), !filePath)
                          return {
                            __symbolic: "error",
                        ...
                        isFunctionParam || (filePath = sourceSymbol.filePath);
                      }
                      return filePath ? self.getStaticSymbol(filePath, name_1) : {
            ...
            try {
              ...
              console.error("Could not resolve module '" + module + "' relative to file " + containingFile), this.reportError(new e, null, containingFile);
            }
          }, StaticSymbolResolver;
        }(),
        AotSummaryResolver = function() {
          function AotSummaryResolver(host, staticSymbolCache) {
            this.host = host, this.staticSymbolCache = staticSymbolCache, this.summaryCache = new Map, this.loadedFilePaths = new Set;
          }
          return AotSummaryResolver.prototype._assertNoMembers = function(symbol) {
            ...
            var _this = this;
            if (!this.loadedFilePaths.has(filePath) && (this.loadedFilePaths.add(filePath), !this.host.isSourceFile(filePath))) {
        ...
        __extends$27 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        AbstractJsEmitterVisitor = function(_super) {
          ...
          return __extends$27(AbstractJsEmitterVisitor, _super), AbstractJsEmitterVisitor.prototype.visitDeclareClassStmt = function(stmt, ctx) {
            var _this = this;
        ...
        __extends$26 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        JitEmitterVisitor = function(_super) {
          ...
          return __extends$26(JitEmitterVisitor, _super), JitEmitterVisitor.prototype.getArgs = function() {
            for (var result = {},
        ...
        __decorate$17 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$17 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
          ...
          return Object.defineProperty(JitCompiler.prototype, "injector", {
            ...
            if (!ngModuleFactory) {
              ...
              compileResult.dependencies.forEach(function(dep) {
                dep.placeholder.reference = _this._assertComponentKnown(dep.comp.reference, !0).proxyComponentFactory;
            ...
            directiveWrapperClass = this._compilerConfig.useJit ? jitStatements("/" + identifierName(moduleMeta.type) + "/" + identifierName(dirMeta.type) + "/wrapper.ngfactory.js", statements, compileResult.dirWrapperClassVar) : interpretStatements(statements, compileResult.dirWrapperClassVar), this._compiledDirectiveWrapperCache.set(dirMeta.type.reference, directiveWrapperClass);
            ...
            if (!template.isCompiled) {
              ...
              var parsedAnimations = this._animationParser.parseComponent(compMeta),
                  ...
                  parsedTemplate = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, template.ngModule.schemas, identifierName(compMeta.type)),
                  compiledAnimations = this._animationCompiler.compile(identifierName(compMeta.type), parsedAnimations),
              ...
              compileResult.dependencies.forEach(function(dep) {
                ...
                if (dep instanceof ViewClassDependency) {
                  var vfd = dep;
                  depTemplate = _this._assertComponentKnown(vfd.comp.reference, !1), vfd.placeholder.reference = depTemplate.proxyViewClass;
                } else if (dep instanceof ComponentFactoryDependency$1) {
                  var cfd = dep;
                  depTemplate = _this._assertComponentKnown(cfd.comp.reference, !0), cfd.placeholder.reference = depTemplate.proxyComponentFactory;
                } else if (dep instanceof DirectiveWrapperDependency) {
                  var dwd = dep;
              ...
              viewClass = this._compilerConfig.useJit ? jitStatements("/" + identifierName(template.ngModule.type) + "/" + identifierName(template.compType) + "/" + (template.isHost ? "host" : "component") + ".ngfactory.js", statements, compileResult.viewClassVar) : interpretStatements(statements, compileResult.viewClassVar), template.compiled(viewClass);
              var _a;
            }
            ...
            return this._resolveStylesCompileResult(result, externalStylesheetsByModuleUrl), this._compilerConfig.useJit ? jitStatements("/" + result.meta.moduleUrl + ".ngstyle.js", result.statements, result.stylesVar) : interpretStatements(result.statements, result.stylesVar);
        ...
        MessageBundle = function() {
          function MessageBundle(_htmlParser, _implicitTags, _implicitAttrs) {
            this._htmlParser = _htmlParser, this._implicitTags = _implicitTags, this._implicitAttrs = _implicitAttrs, this._messages = [];
          }
        ...
        Extractor = function() {
          ...
          return Extractor.prototype.extract = function(rootFiles) {
            var _this = this,
                ...
                files = (_a.ngModuleByPipeOrDirective, _a.files),
                ngModules = _a.ngModules;
            return Promise.all(ngModules.map(function(ngModule) {
            ...
            var config = new CompilerConfig({
              ...
              useJit: !1
                ...
                elementSchemaRegistry = new DomElementSchemaRegistry,
                resolver = new CompileMetadataResolver(new NgModuleResolver(staticReflector), new DirectiveResolver(staticReflector), new PipeResolver(staticReflector), summaryResolver, elementSchemaRegistry, normalizer, staticReflector),
                messageBundle = new MessageBundle(htmlParser, [], {}),
        ...
        __decorate$18 = this && this.__decorate || function(decorators, target, key, desc) {
          var d,
              c = arguments.length,
              r = c < 3 ? target : null === desc ? desc = Object.getOwnPropertyDescriptor(target, key) : desc;
          if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
        ...
        __metadata$18 = this && this.__metadata || function(k, v) {
          if ("object" == typeof Reflect && "function" == typeof Reflect.metadata)
            return Reflect.metadata(k, v);
        },
